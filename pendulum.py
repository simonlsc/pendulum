...
def getline(Point1: Mobject,Point2: Mobject):
...
class SimplePendulum(Dot):
    def __init__(self, length, weight, theta0, w0, colour):
        ...
        self.mass = Dot(radius=0.04*weight).move_to(self.x1[0]*RIGHT+self.y1[0]*UP).set_color(colour)
        self.line = getline(self,self.mass)
        self.line.add_updater(lambda mob: mob.become(getline(self,self.mass)))
...
class DoublePendulum():
    def __init__(self,SP1: SimplePendulum,SP2: SimplePendulum):
        ...
        SP2.line.add_updater(lambda mob: mob.become(getline(SP1.mass,SP2.mass)))
...
class MultiPendulums(Scene):
    def construct(self):
        ...
        #Pendulums Motions
        lengthList = np.arange(0.5,3.5,0.5)
        weightList = [3.0]*len(lengthList)
        theta0List = [170.0]*len(lengthList)
        w0List = [0]*len(lengthList)
        
        # lengthList = [1,1,1,  1,1,1.5,2,2.5,3]
        # weightList = [3,2,1,0.5,1,1,1,1,1]
        # theta0List = [170.0]*len(lengthList)
        # w0List = [-10,-5,0,5,10,0,0,0,0]
        colourList = color_gradient([PURE_RED,YELLOW,PURE_GREEN,PURE_BLUE,PURPLE],len(lengthList))

        pGroup = VGroup(*list(map(lambda l,w,t,w0,c: SimplePendulum(l,w,t,w0,c),lengthList,weightList,theta0List,w0List,colourList))).arrange(buff=1)
        self.add(pGroup)
